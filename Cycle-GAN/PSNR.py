...
parser.add_argument("--model", default="cycleGan/Experiment/exp_512/checkpoint/model_denoise_10_30.pth", type=str, help="model path")
#parser.add_argument("--model", default="cycleGan/Experiment/exp2/checkpoint/model_denoise_163_50.pth", type=str, help="model path")
# parser.add_argument("--save", default="cycleGan/NR_result", type=str, help="savepath, Default: results")
# parser.add_argument("--ori", default="cycleGan/NR_result", type=str, help="savepath, Default: results")
...
parser.add_argument("--save", default="cycleGan/ep10", type=str, help="savepath, Default: results")
parser.add_argument("--ori", default="cycleGan/ep10", type=str, help="savepath, Default: results")
parser.add_argument("--gpus", default="0", type=str, help="gpu ids")
parser.add_argument("--root", default="dataset/preprocessed/ALLDATA", type=str)
...
training_data_loader = DataLoader(dataset=train_set,batch_size=1,shuffle=False)
...
with torch.no_grad():
    for iteration, batch in enumerate(training_data_loader):
        ...
        #im_output = torch.clamp(im_output,min=0.0,max=1.0)
        pp=PSNR(im_output,im_input)
        #print(pp)
        im_output = im_output.cpu()
        # # save_image(im_output.data,'6.png')
        save_image(im_output.data,opt.save+'/'+str(pp)+'__'+name[-1])
        ...
        # save_image(result.cpu().data,opt.save+'/'+name[-1])