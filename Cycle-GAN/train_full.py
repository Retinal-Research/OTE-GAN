...
def main():
    ...
    file = open('cycleGan/Experiment/exp_full/checksample/mse_'+str(opt.nEpochs)+'_'+str(opt.sigma)+'.txt','w')
    for g in all_g:
        file.write(g+'\n')
    file.close()

    file = open('cycleGan/Experiment/exp_full/checksample/Gloss_'+str(opt.nEpochs)+'_'+str(opt.sigma)+'.txt', 'w')
    for d in all_d:
        file.write(d + '\n')
    file.close()
    # psnr = eval_dep(model)
...
def train(training_data_loader,criterion_GAN,criterion_cycle,criterion_identity,optimizer_G, optimizer_D_A,optimizer_D_B, g_AB,g_BA, discr_A, discr_B, epoch,num_rand):
    ...
    save_image(fake_B.data, 'cycleGan/Experiment/exp_full/checksample/'+str(epoch)+'_'+str(num_rand)+'_'+'output.png')
    save_image(real_A.data, 'cycleGan/Experiment/exp_full/checksample/'+str(epoch)+'_'+str(num_rand)+'_'+'input.png')
    save_image(real_B.data, 'cycleGan/Experiment/exp_full/checksample/'+str(epoch)+'_'+str(num_rand)+'_'+'gt.png')
...
def save_checkpoint(model, discr, epoch):
    model_out_path = "cycleGan/Experiment/exp_full/checkpoint/" + "model_denoise_"+str(epoch)+"_"+str(opt.sigma)+".pth"
    state = {"epoch": epoch ,"model": model, "discr": discr}
    if not os.path.exists("cycleGan/Experiment/exp_full/checkpoint/"):
        os.makedirs("cycleGan/Experiment/exp_full/checkpoint/")
