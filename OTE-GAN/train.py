...
def train(training_data_loader, G_optimizer, D_optimizer, model, discr, epoch,num_rand):
    ...
    save_image(G_result.data, 'OTE-GAN/Experiment/exp11/checksample/'+str(epoch)+'_'+str(num_rand)+'_'+'output.png')
    save_image(input.data, 'OTE-GAN/Experiment/exp11/checksample/'+str(epoch)+'_'+str(num_rand)+'_'+'input.png')
    save_image(target.data, 'OTE-GAN/Experiment/exp11/checksample/'+str(epoch)+'_'+str(num_rand)+'_'+'gt.png')
...
def save_checkpoint(model, discr, epoch):
    model_out_path = "OTE-GAN/Experiment/exp11/checkpoint/" + "model_denoise_"+str(epoch)+"_"+str(opt.sigma)+".pth"
    state = {"epoch": epoch ,"model": model, "discr": discr}
    if not os.path.exists("OTE-GAN/Experiment/exp11/checkpoint/"):
        os.makedirs("OTE-GAN/Experiment/exp11/checkpoint/")
