...
def main():
    ...
    file = open('OttGam/Experiment/exp_512/checksample/mse_'+str(opt.nEpochs)+'_'+str(opt.sigma)+'.txt','w')
    for mse in MSE:
        file.write(mse+'\n')
    file.close()

    file = open('OttGam/Experiment/exp_512/checksample/Gloss_'+str(opt.nEpochs)+'_'+str(opt.sigma)+'.txt', 'w')
    for g in GLOSS:
        file.write(g + '\n')
    file.close()
    # psnr = eval_dep(model)
...
def train(training_data_loader, G_optimizer, D_optimizer, model, discr, criterion, epoch,num_rand):
    ...
    save_image(G_result.data, 'OttGam/Experiment/exp_512/checksample/'+str(epoch)+'_'+str(num_rand)+'_'+'output.png')
    save_image(input.data, 'OttGam/Experiment/exp_512/checksample/'+str(epoch)+'_'+str(num_rand)+'_'+'input.png')
    save_image(target.data, 'OttGam/Experiment/exp_512/checksample/'+str(epoch)+'_'+str(num_rand)+'_'+'gt.png')


    return torch.mean(torch.FloatTensor(mse)),torch.mean(torch.FloatTensor(Gloss))
   
def save_checkpoint(model, discr, epoch):
    model_out_path = "OttGam/Experiment/exp_512/checkpoint/" + "model_denoise_"+str(epoch)+"_"+str(opt.sigma)+".pth"
    state = {"epoch": epoch ,"model": model, "discr": discr}
    if not os.path.exists("OttGam/Experiment/exp_512/checkpoint/"):
        os.makedirs("OttGam/Experiment/exp_512/checkpoint/")
